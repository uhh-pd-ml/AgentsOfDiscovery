...
#SBATCH --job-name=control_gpt-4.1       # job name
#SBATCH --output=log/only_ml_control/b1_gpt-41-%A_%a.out      # log file for command line output
#SBATCH --error=log/only_ml_control/b1_gpt-41-%A_%a.err       # log file for errors and warnings
...
PROJECTPATH=project_descriptions/anomaly_c_w_bkg_only_ml_control/
MODEL=gpt-4.1
WORKDIR=/out/batched/only_ml_control/$(date '+%Y-%m-%d')
...
cd code/mt/masterthesis/scripts/ \
    ...
    --work_dir $WORKDIR \
    --model $MODEL \
    --evaluation_param_file $PROJECTPATH/eval.json \
